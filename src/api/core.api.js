...
import {
	mapJobApplicationDetailsFromAPI,
	mapJobApplicationFromAPI,
	mapMyApplicationDetailsFromAPI,
	mapMyApplicationFromAPI,
} from "./mappers/application.mapper.js";

import { mapContractFromAPI } from "./mappers/contract.mapper.js";

import {
	mapJobDetailsPageFromAPI,
	mapJobFromAPI,
	mapJobListItemFromAPI,
	mapJobSummaryFromAPI,
	mapJobToAPI,
	mapPaginationFromAPI,
} from "./mappers/job.mapper.js";

import { mapCounterOfferToAPI } from "./mappers/negotiation.mapper.js";
...
import {
	...
	mapSkillsFromAPI,
} from "./mappers/profile.mapper.js";

import { mapReviewDataFromAPI, mapReviewToAPI } from "./mappers/review.mapper.js";

import { mapApplicationStatusFromAPI, mapContractStatusFromAPI } from "./mappers/status.mapper.js";
...
export async function getMyProfile() {
	...
	const { profile, reviews, skills = [] } = data;
...
export async function getAllJobs({ search, filters, page, pageSize }, signal) {
...
export async function getMyApplicationDetails(applicationId, signal) {
	const { data } = await api.get(`/applications/${applicationId}/me`, {
		signal,
	});
...
export async function createJobApplication(jobId, applicationData) {
	const { data } = await api.post(`/jobs/${jobId}/applications`, {
...
export async function decideJobApplication(jobId, applicationId, decision) {
	const { data } = await api.patch(`/jobs/${jobId}/applications/${applicationId}/decision`, {
		decision,
	});
...
export async function acceptNegotiationTerms(jobId, applicationId) {
	const { data } = await api.post(`/jobs/${jobId}/applications/${applicationId}/accept`);
...
export async function rejectNegotiationTerms(jobId, applicationId) {
	const { data } = await api.post(`/jobs/${jobId}/applications/${applicationId}/reject`);
...
export async function submitReview(jobId, review) {
	const { data } = await api.post(`/jobs/${jobId}/review`, mapReviewToAPI(review));
